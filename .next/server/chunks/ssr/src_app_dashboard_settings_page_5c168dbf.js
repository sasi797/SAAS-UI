...
// === Predefined Fields (nested: tabs -> sections -> fields)
// const PRESET_FIELDS = {
//   Driver: {
//     tabs: [
//       {
...
//                 rules: { minLength: "3", maxLength: "12", pattern: "" },
//                 conditions: [
//                   {
//                     fieldId: "Status",
//                     operator: "equals",
...
//                     actions: [
//                       {
//                         type: "setRequiredFalse",
//                         target: "Phone / Mobile Number",
//                       },
//                       { type: "disableField", target: "Email ID" },
//                     ],
//                   },
//                 ],
//               },
...
//                 disabled: true,
//                 roles: ["admin", "superuser"],
//                 rules: { minLength: "5", maxLength: "20", pattern: "" },
//                 conditions: [
//                   {
//                     fieldId: "Driver ID",
//                     operator: "notEmpty",
//                     value: "",
//                     actions: [
//                       { type: "disableField", target: "Driver ID" },
//                       {
//                         type: "setRequiredFalse",
//                         target: "Phone / Mobile Number",
//                       },
//                       { type: "hideField", target: "Email ID" },
//                     ],
//                   },
//                 ],
//               },
...
//                 disabled: true,
//                 roles: ["admin", "superuser"],
//                 rules: { minLength: "3", maxLength: "50", pattern: "" },
//                 conditions: [
//                   {
//                     fieldId: "Status",
//                     operator: "equals",
...
//                 },
//                 conditions: [
//                   { fieldId: "Full Name", operator: "notEmpty", value: "" },
//                 ],
//               },
//               {
//                 label: "Email ID",
...
//                 rules: {},
//                 conditions: [
//                   { fieldId: "Driver ID", operator: "notEmpty", value: "" },
//                 ],
//               },
//             ],
//           },
...
//                 disabled: true,
//                 roles: ["admin", "superuser"],
//                 rules: { minLength: "3", maxLength: "50" },
//                 conditions: [
//                   { fieldId: "Location ID", operator: "notEmpty", value: "" },
//                 ],
//               },
//               {
//                 label: "Client Name",
...
//                 conditions: [
//                   { fieldId: "City", operator: "notEmpty", value: "" },
//                 ],
//               },
//               {
//                 label: "City",
...
//                 rules: { minLength: "", maxLength: "" },
//                 conditions: [
//                   { fieldId: "State", operator: "equals", value: "TN" },
//                 ],
//               },
//             ],
//           },
...
function ModuleDynamicFormBuilder() {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        const getPresetFields = async ()=>{
            ...
            try {
                const result = await getApi("http://localhost:8000/fieldindex00?entity_name=Location");
                if (result?.data?.data) {
                    // ✅ Map or transform if needed
                    const fields = result.data.data.map((item)=>({
                            ...item,
                            key: item.id
                        }));
    ...
    // === Initialize formState with nested structure and generated ids
    const [formState, setFormState] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useState"])(()=>{
        ...
        MASTERS.forEach((m)=>{
            ...
            // Fix condition fieldIds to reference generated ids where possible (within same master)
            // Build a flat lookup of label -> id for this master
            const flat = tabs.flatMap((tab)=>tab.sections.flatMap((sec)=>sec.fields));
            flat.forEach((fld)=>{
                fld.conditions = (fld.conditions || []).map((c)=>{
                    // if fieldId matches a label in this master, replace with its id
    ...
    // selectedFieldId: derive first nested field id as default
    const [selectedFieldId, setSelectedFieldId] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useState"])(()=>{
        const master = formState[MASTERS[0]];
    ...
    const updateMaster = (master, updater)=>{
        setFormState((prev)=>{
            const next = {
                ...prev
            };
            next[master] = updater(prev[master]);
            return next;
        });
    };
    // -----------------------
    // Flatten nested tabs -> sections -> fields for UI rendering and lookups
    // -----------------------
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        // select first field of the currently active master when master or formState changes
        const master = formState[activeMaster];
        const firstField = master?.tabs?.[0]?.sections?.[0]?.fields?.[0] || null;
        setSelectedFieldId((prev)=>{
            // if previous selected belongs to this master and still exists, keep it
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        ...
        // when selecting a field programmatically, clear unsaved flag (user hasn't edited yet)
    ...
    // === Unsaved changes dialog state + helper ===
    ...
    // helper that marks dirty and updates editorField;
    // accepts either an object patch or an updater function (like setState callback)
    ...
    // -----------------------
    // other helpers from your original code (kept)
    // -----------------------
    // const applyEditorSave = () => { ... } <-- we'll override with async below
    ...
    function getFieldChanges(original, updated) {
        const changes = {};
        Object.keys(updated).forEach((key)=>{
            ...
            if (Array.isArray(newVal)) {
                ...
                const nested = getFieldChanges(oldVal || {}, newVal);
    ...
    // === Handle selecting a field from the left panel (with unsaved changes prompt) ===
    const handleFieldSelect = (field)=>{
        ...
        // setEditorField to real copy already handled by effect watching selectedFieldId
        }
    };
    // === applyEditorSave: build payload and PUT to backend; update local state ===
    const applyEditorSave = async ()=>{
        if (!editorField) return;
        // find tab/section info for payload
    ...
    // Dialog actions for unsaved changes
    const handleDiscardAndSwitch = ()=>{
        if (!pendingField) {
            setShowUnsavedDialog(false);
            return;
        }
        // discard current local edits and switch to pending field
        setHasUnsavedChanges(false);
        setShowUnsavedDialog(false);
...
//# sourceMappingURL=src_app_dashboard_settings_page_5c168dbf.js.map