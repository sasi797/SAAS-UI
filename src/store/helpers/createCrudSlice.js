...
export function createCrudSlice({ name, endpoint }) {
    // 🔹 CRUD Thunks
    const getAll = createAsyncThunk(`${name}/getAll`, async (_, { rejectWithValue }) => {
        try {
    ...
    const getById = createAsyncThunk(`${name}/getById`, async (id, { rejectWithValue }) => {
        try {
            return await getApi(`${endpoint}/${id}`);
        } catch (err) {
            return rejectWithValue(err.message);
        }
    });

    const createItem = createAsyncThunk(`${name}/create`, async (data, { rejectWithValue }) => {
        try {
    ...
    const updateItem = createAsyncThunk(`${name}/update`, async ({ id, data }, { rejectWithValue }) => {
        try {
            return await putApi(`${endpoint}/${id}`, data);
        } catch (err) {
            return rejectWithValue(err.message);
        }
    });
